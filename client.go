...
//ID returns and immutable copy of the ID of this client, which is established ince the client has succesfullt
//completed the handshake protocol configured from this clients associated node
func (c *Client) ID() ID {
	return c.id
}

//Logger returns the underlying logger associated to this client. It may optionally set via (*Client).SetLogger
...
//WaitUntilReady pauses the goroutine to which it was called within until the client has successfully
//completed the handshak protocol configured under the node instance to which this peer was derived from
...
// has been closed. The goroutine associated to this client would only close should
//
...
func (c *Client) handshake() {
	...
	if len(data) != SizePublicKey+SizeSignature {
		c.reportError(fmt.Errorf("Received invalid number of bytes opneing a session: expected %d byte(s). but got %d byte(s)", SizePublicKey+SizeSignature, len(data)))
		return
	}
	...
	if err != nil {
		c.reportError(fmt.Errorf("Could not instantiatte aes: %w", err))
		return
	}

	suite, err := cipher.NewGCM(core)
	if err != nil {
		c.reportError(fmt.Errorf("Could not instantiatte aes-gcm: %w", err))
		return
	}
	...
	if err := c.write(buf); err != nil {
		c.reportError(fmt.Errorf("Failed to sen session handshake: %w", err))
		return
	}
	...
	if err != nil {
		c.reportError(fmt.Errorf("Failed to parse ID while handling overlay handshake: %w", err))
		return
	}
	...
	fmt.Println(SizeSignature)
	if len(data) != len(buf)+SizeSignature {
		c.reportError(fmt.Errorf("Received invalid number of bytes handshaking: expected %d byte(s), got %d byte(s)", len(buf)+SizeSignature, len(data)))
		return
	}
...
func (c *Client) recvLoop() {
	...
	for {
		...
		if err != nil {
			if !isEOF(err) {
				c.Logger().Warn("Got and error while sending messages.", zap.Error(err))
			}
			c.reportError(err)
			break
		}
		...
		if err != nil {
			c.Logger().Warn("Got an error reading incoming messages.", zap.Error(err))
			c.reportError(err)
			break
		}
	...
	for {
		...
		for _, msg := range writerBuf {
			...
			if _, err := c.writer.Write(header); err != nil {
				if !isEOF(err) {
					c.Logger().Warn("Got and error writing header.", zap.Error(err))
				}
				c.reportError(err)
