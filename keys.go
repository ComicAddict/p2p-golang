...
func LoadKeysFromHex(secretHex string) (PrivateKey, error) {
	...
	if len(secret) != SizePrivateKey {
		return ZeroPrivateKey, fmt.Errorf("got private key od %d byte(s), but expected %d bytes(s): %w", len(secret), SizePrivateKey, io.ErrUnexpectedEOF)
	}
...
//MarshalJSON returns the hexdecimal reperesentation of this private key in JDON. It should never throw an error
...
// MarshalJSON returns the hexadecimal representation of this signature inJSON. It should never throw an error
func (s Signature) MarshalJSON() ([]byte, error) {
	return json.Marshal(s.String())
}

//UnmarshalSignature decodes data into a Signature instance
